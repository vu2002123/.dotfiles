...
-- Use Treesitter for folding (makes it understand code structure)
vim.opt.foldmethod = 'expr'
vim.opt.foldexpr = 'v:lua.vim.treesitter.foldexpr()'

-- Start with all folds OPEN (so you don't open a file and see nothing)
vim.opt.foldenable = false
-- OR start with them CLOSED but only at high levels:
...
require('lazy').setup({
    ...
    config = function()
      ...
      vim.api.nvim_create_autocmd('LspAttach', {
        ...
        callback = function(event)
          ...
          -- --- SNACKS.PICKER REPLACEMENTS BELOW ---
          ...
          end, '[G]oto [O]utline (Document Symbols)')

          -- Fuzzy find all the symbols in your current workspace.
          map('gW', function()
            Snacks.picker.lsp_workspace_symbols()
          end, '[G]oto [W]orkspace Symbols')
          ...
          -- --- END SNACKS.PICKER REPLACEMENTS ---

          -- The following two autocommands are used to highlight references of the
          -- word under your cursor when your cursor rests there for a little while.
          local client = vim.lsp.get_client_by_id(event.data.client_id)

          -- Helper function for version compatibility (0.10 vs 0.11)
      ...
      local servers = {
        ...
        mason = {
          ...
          --
          basedpyright = {
            -- 1. Settings to control the Analysis engine
            settings = {
              basedpyright = {
                -- Stop basedpyright from trying to sort imports (Ruff is better at this)
                disableOrganizeImports = true,
                analysis = {
                  -- Your requested mode
                  typeCheckingMode = 'basic',
                  -- Optional: Disable diagnostics that overlap with Ruff to avoid double-warnings
                  reportUnusedImport = 'none',
                  reportUnusedVariable = 'none',
                },
              },
            },
            -- 2. The "Clash" Fix: Disable Formatting
            -- This function runs when the server attaches to the buffer.
            on_attach = function(client, bufnr)
              -- Tell Neovim: "This server cannot format code."
          ...
          ruff = {
            -- Ruff is purely for linting/formatting.
            -- It usually takes priority automatically, but the setup above guarantees it.
            on_attach = function(client, bufnr)
              -- Optional: Disable hover if you find it annoying/duplicate with pyright
  ...
  { -- Highlight, edit, and navigate code
    ...
    build = ':TSUpdate',
    config = function()
      local config = require 'nvim-treesitter.configs' -- Sets main module to use for opts
